...
setup_analysis <- function(report_date, training_period, forecast_horizon, output_dir=NULL, local_cache_dir=NULL){
  ...
  # Write output variables to a yml file
  output_file_path <- paste0(out_dir_name, "/setup-",Sys.Date(), ".yml")
  output_data <- list(
    forecast_date = forecast_date,
    train_start_date = train_start_date,
    train_end_date = train_end_date,
    forecast_horiz_start = forecast_horiz_start,
    forecast_horiz_end = forecast_horiz_end,
    local_cache_dir = local_cache_dir,
    full_time_span = full_time_span,
    output_dir = output_dir,
    out_dir_name = out_dir_name
  )
  write_yaml(output_data, output_file_path)
  su_yaml <<- read_yaml(output_file_path)
