...
#' @param y A numeric vector of dates to search for matches.
#' @return A numeric vector of dates from \code{y} that are nearest matches for dates given in \code{x}
#' @examples
...
#' MyResult <- match_biweek(observed_dates, seq(min(observed_dates), max(observed_dates), by = "14 days"))
#' MyResult
